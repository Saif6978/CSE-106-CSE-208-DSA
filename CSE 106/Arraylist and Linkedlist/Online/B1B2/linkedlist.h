...
void swap_with(LinkedList* list, int pos) {
    if (pos < 0 || pos >= list->size) {
        printf("Invalid position\n");
        return;
    }
    Node* other = list->head;
    for (int i = 0; i < pos; i++) other = other->next;
    int temp = list->current_node->data;
    list->current_node->data = other->data;
    other->data = temp;
    printf("Swapped with position %d\n", pos);
}
